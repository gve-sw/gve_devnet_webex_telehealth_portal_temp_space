...
router.post('/outpatient', async function(req, res, next) {
  if(req.session.type == "caregiver"){
    ...
    webex.rooms.create({
      title: `Appointment with ${req.session.user}`
    }).then(function(room){
  ...
  if(req.session.type == "caregiver"){
    Outpatient.find({ uuid: req.params.uuid }, (err, result) =>{
      ...
      for(var i=0; i<consultations.length; i++){
        if(((parseInt(consultations[i].date_time) - Date.now())/1000)/60 < 5 && ((parseInt(consultations[i].date_time) - Date.now())/1000)/60 > -5 ){
          res.render('caregiver_consultation', {
            patient: result[0],
            access_token: req.session.access_token,
            destination: consultations[i].room_id,
